...
import { hexToRgb, hslToRgb, randomHsl, rgbToHex } from "@/utils/colors";
...
const generateSimilarHex = (baseHex: string, difficulty: Difficulty): string => {
  ...
  switch (difficulty) {
    case "easy":
      variation = randomInt(60, 100);
      break;
    case "medium":
      variation = randomInt(30, 50);
      break;
    case "hard":
      variation = randomInt(10, 25);
      break;
    case "expert":
      variation = randomInt(6, 14);
      break;
  }
  ...
  const handleSelect = useCallback(
    (index: number) => {
      ...
      if (correct) {
        ...
        const explanation =
          language === "ru"
            ? `HEX-код состоит из трёх пар цифр (RR GG BB). Правильный: R=${correctRgb.r} G=${correctRgb.g} B=${correctRgb.b}. Ваш выбор: R=${userRgb.r} G=${userRgb.g} B=${userRgb.b}. Разница в каналах помогает различать близкие цвета.`
        ...
        addMistake({
          question: language === "ru" ? "Угадать HEX-код цвета" : "Guess the HEX color code",
    ...
    <div className="space-y-6">
      <div className="text-center space-y-2">
        <h2 className="text-xl sm:text-2xl font-display font-semibold tracking-tight">
          {language === "ru" ? "Угадать HEX-код" : "Guess the HEX code"}
        </h2>
        <HintToggle
          hint={
            language === "ru"
              ? "HEX состоит из трёх пар (RR GG BB). Сравните тон и яркость со свотчем."
