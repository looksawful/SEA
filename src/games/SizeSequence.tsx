...
import { localize, LocalizedText, t } from "@/utils/i18n";
...
const SCALE_NAMES: Record<ScaleType, LocalizedText> = {
  ...
  "major-third": text("большая терция (1.25)", "major third"),
  "perfect-fourth": text("чистая кварта (1.333)", "perfect fourth"),
};
  ...
  const handleSelect = useCallback(
    (index: number) => {
      ...
      if (correct) {
        ...
        addMistake({
          question:
            language === "ru" ? `Найти ошибку в ${scaleName} шкале` : `Find the error in the ${scaleName} scale`,
          userAnswer:
            language === "ru"
    ...
    <div className="space-y-6">
      <div className="text-center space-y-2">
        <h2 className="text-xl sm:text-2xl font-display font-semibold tracking-tight">
          {language === "ru" ? "Найти ошибку в шкале" : "Find the error in the scale"}
        </h2>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-4">
                <span className="w-8 text-center font-mono text-sm text-soft">{index + 1}</span>
                <motion.span
